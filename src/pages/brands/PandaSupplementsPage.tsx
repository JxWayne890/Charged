  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const sortedProducts = pandaProducts.sort((a, b) => a.title.localeCompare(b.title));
        console.log(`Found ${sortedProducts.length} Panda Supplements products`);
        setProducts(sortedProducts);
        setError(null);
      } catch (err) {
        console.error('Failed to load Panda Supplements products:', err);
        setError('Failed to load products. Please try again later.');
        toast({
          title: "Error",
          description: "Failed to load Panda Supplements products",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
    ...
    <div className="container mx-auto px-4 py-8 mt-20">
      ...
      {products.length > 0 ? (
        ...
        <div className="text-center py-12">
          <p className="text-gray-600 text-lg">No Panda Supplements products available at the moment. Check back soon!</p>
        </div>
      )}
    </div>
  );
...
export default PandaSupplementsPage;
